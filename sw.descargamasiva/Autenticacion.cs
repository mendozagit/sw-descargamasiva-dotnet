    ...
    internal class Autenticacion : SoapRequestBase
    {
        ...
        public string Generate(X509Certificate2 certificate)
        {
            ...
            string soap_request =
                ...
                @"<o:Reference ValueType=""http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-x509-token-profile-1.0#X509v3"" URI=""#" +
                uuid + @"""/>" +
                @"</o:SecurityTokenReference>" +
                @"</KeyInfo>" +
                @"</Signature>" +
        ...
        public override StepResponse GetResult(XmlDocument xmlDoc, string step)
        {
            ...
            {
                Token = xmlDoc.GetElementsByTagName("AutenticaResult")[0]?.InnerXml,
                RawRequest = CacheManager.RawRequest,
                RawResponse = CacheManager.RawResponse,
                Step = "Step1GetToken"
            };
            return stepResponse;
        }
